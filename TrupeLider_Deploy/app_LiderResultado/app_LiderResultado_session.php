<?php
   ...
   if ($NM_session_db && !defined('NM_CONTR_SESS_DB'))
   {
       ...
       {
           $NM_session_conf = substr($NM_session_prod, 0, strrpos($NM_session_prod, '/')) . "/conf";
       }
       include_once($NM_session_prod . "/lib/php/nm_session.php");
       $sc_session  = new sc_session();
       $sc_sess_sch = $NM_session_sch;
       $sc_sess_db  = $sc_session->conect($NM_session_conex, $NM_session_tab, $NM_session_prod, $NM_session_conf);
       if ($sc_session->testTable($sc_sess_db, $NM_session_sch))
       {
