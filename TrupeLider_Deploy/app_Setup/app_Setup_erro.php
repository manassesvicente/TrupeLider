...
$old_error_handler = set_error_handler("app_Setup_scriptcase_error_handler");

function app_Setup_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['app_Setup']['contr_erro']) && $_SESSION['scriptcase']['app_Setup']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            if ($inicial_app_Setup->contr_app_Setup->NM_ajax_flag)
            {
                $inicial_app_Setup->contr_app_Setup->NM_ajax_info['errList']['geral_app_Setup'][] = $msg;
                return;
            }
            app_Setup_scriptcase_error_display($msg, $err_no);
        }
...
function app_Setup_scriptcase_error_display($err_msg, $err_no)
{
?>
