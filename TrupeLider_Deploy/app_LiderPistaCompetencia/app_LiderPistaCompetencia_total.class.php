...
class app_LiderPistaCompetencia_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   //----- Construtor
   function app_LiderPistaCompetencia_total($sc_page)
   {
      $this->sc_page = $sc_page;
      $this->nm_data = new nm_data("pt_br");
      if (!empty($_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']))
      { 
          $this->lider = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['lider']; 
          $tmp_pos = strpos($this->lider, "##@@");
          if ($tmp_pos !== false)
          {
              $this->lider = substr($this->lider, 0, $tmp_pos);
          }
          $this->pista = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['pista']; 
          $tmp_pos = strpos($this->pista, "##@@");
          if ($tmp_pos !== false)
          {
              $this->pista = substr($this->pista, 0, $tmp_pos);
          }
          $this->competencia = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['competencia']; 
          $tmp_pos = strpos($this->competencia, "##@@");
          if ($tmp_pos !== false)
          {
              $this->competencia = substr($this->competencia, 0, $tmp_pos);
          }
          $this->nota_media = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['nota_media']; 
          $tmp_pos = strpos($this->nota_media, "##@@");
          if ($tmp_pos !== false)
          {
              $this->nota_media = substr($this->nota_media, 0, $tmp_pos);
          }
          $this->resultado_medio = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['resultado_medio']; 
          $tmp_pos = strpos($this->resultado_medio, "##@@");
          if ($tmp_pos !== false)
          {
              $this->resultado_medio = substr($this->resultado_medio, 0, $tmp_pos);
          }
          $this->total_nota = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['total_nota']; 
          $tmp_pos = strpos($this->total_nota, "##@@");
          if ($tmp_pos !== false)
          {
              $this->total_nota = substr($this->total_nota, 0, $tmp_pos);
          }
          $this->total_resultado = $_SESSION['sc_session'][$this->sc_page]['app_LiderPistaCompetencia']['campos_busca']['total_resultado']; 
   ...
   {
      global $nada ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(NOTA_MEDIA) as tot_nota_media, sum(RESULTADO_MEDIO) as tot_resultado_medio, sum(TOTAL_NOTA) as tot_total_nota, sum(TOTAL_RESULTADO) as tot_total_resultado from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(NOTA_MEDIA) as tot_nota_media, sum(RESULTADO_MEDIO) as tot_resultado_medio, sum(TOTAL_NOTA) as tot_total_nota, sum(TOTAL_RESULTADO) as tot_total_resultado from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][0] = "Total Geral" ; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['tot_geral'][5] = $rt->fields[4]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['app_LiderPistaCompetencia']['contr_total_geral'] = "OK";
   } 
   ...
   function resumo($destino_resumo, &$array_total_lider, &$array_total_pista)
   {
      global $nada;
   ...
   $nmgp_order_by = " order by LIDER asc, PISTA asc"; 
      ...
      { 
         $comando  = "select count(*), sum(NOTA_MEDIA) as tot_nota_media, sum(RESULTADO_MEDIO) as tot_resultado_medio, sum(TOTAL_NOTA) as tot_total_nota, sum(TOTAL_RESULTADO) as tot_total_resultado, LIDER, PISTA from " . $this->Ini->nm_tabela . " " .  $this->sc_where_atual . " group by LIDER, PISTA $nmgp_order_by";
      } 
   ...
   function grafico(&$array_label, &$array_datay, $array_total_lider, $array_total_pista)
   {
      $array_label    = array();
      $array_label[0] = array();
      $array_label[1] = array();
