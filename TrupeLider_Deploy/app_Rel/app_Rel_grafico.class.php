...
class app_Rel_grafico
{
   ...
   function monta_grafico()
   {
      ...
      {
          $graf_tipo    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_tipo'];
          $graf_alt     = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_alt'];
          $graf_larg    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_larg'];
          $graf_marg    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_margem'];
          $graf_asp     = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_aspec'];
          $graf_preen   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_preencher'];
          $graf_ex_val  = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_exibe_val'];
          $graf_ex_marc = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_exibe_marcas'];
          $this->graf_cor_fundo   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_cor_fundo'];
          $this->graf_cor_margens = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_cor_margens'];
          $this->graf_cor_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_cor_label'];
          $this->graf_cor_valores = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_cor_valores'];
          $this->graf_tipo_marcas = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_tipo_marcas'];
          $this->graf_angulo      = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_angulo'];
   ...
   {
      global 
             $nivel_quebra, $campo, $id_votado, $id_etapa, $id_competencia;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['graf_opc_atual'] == 1)
      {
         $this->sc_graf_sint = true;
      }
      //---- Total
      require_once($this->Ini->path_aplicacao . "app_Rel_total.class.php"); 
      $this->total = new app_Rel_total($this->Ini->sc_page);
      $this->prep_modulos("total");
      //---- Parâmetros
      if (get_magic_quotes_gpc())
      {
         $id_votado = stripslashes($id_votado);
         $id_etapa = stripslashes($id_etapa);
         $id_competencia = stripslashes($id_competencia);
      }
      $id_votado = urldecode($id_votado);
      $id_etapa = urldecode($id_etapa);
      $id_competencia = urldecode($id_competencia);
      $this->campo = (isset($campo))        ? $campo        : 0;
      $this->nivel = (isset($nivel_quebra)) ? $nivel_quebra : 0;
      $this->id_votado = (isset($id_votado)) ? $id_votado : "";
      $this->id_etapa = (isset($id_etapa)) ? $id_etapa : "";
      $this->id_competencia = (isset($id_competencia)) ? $id_competencia : "";
      //---- Arrays de totais
      ...
      $this->list_decimais[1] = 2;
      $this->list_titulo[1]   = "RESULTADO";
      ...
      if (0 < $this->nivel)
      {
         $this->titulo .= " - Votado = $conteudo";
      }
      $conteudo = $id_etapa;
      $id_etapa = $id_etapa;
      if (1 < $this->nivel)
      {
         $this->titulo .= " - Etapa = $conteudo";
      }
      //---- Label
      $this->label    = array();
      $this->label[0] = "Votado";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_votado']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_votado']))
      {
         $this->label[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_votado'];
      }
      $this->label[1] = "Etapa";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_etapa']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_etapa']))
      {
         $this->label[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_etapa'];
      }
      $this->label[2] = "Competência";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_competencia']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_competencia']))
      {
         $this->label[2] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['contr_label_graf']['id_competencia'];
      }
   }
   ...
   function monta_arrays()
   {
      ...
      $this->total->resumo("gra", $this->array_total_id_votado, $this->array_total_id_etapa, $this->array_total_id_competencia);
      ksort($this->array_total_id_votado);
      ksort($this->array_total_id_etapa);
      ksort($this->array_total_id_competencia);
      $this->total->grafico($this->array_label_geral, $this->array_datay_geral, $this->array_total_id_votado, $this->array_total_id_etapa, $this->array_total_id_competencia);
      if ($this->nivel == 0)
      {
         ...
         else
         {
             ...
             foreach ($this->array_label as $lab_qb1)
             {
                 ...
                 {
                     if (isset($this->array_total_id_etapa[$lab_qb1][$xlab_qb2]))
                     {
                         $this->array_datay[$xlab_qb2][] =  $this->array_total_id_etapa[$lab_qb1][$xlab_qb2][$this->campo];
                     }
                     else
                     {
      ...
      if ($this->nivel == 1)
      {
         $this->array_label = $this->array_label_geral[$this->nivel][$this->id_votado];
         if ($this->sc_graf_sint)
         {
             $this->array_datay[0] = $this->array_datay_geral[$this->nivel][$this->campo][$this->id_votado];
         }
         else
         {
             $nn = $this->nivel + 1;
             ...
             foreach ($this->array_label as $lab_qb1)
             {
                 foreach ($lab_qb2 as $xlab_qb2)
                 {
                     if (isset($this->array_total_id_competencia[$lab_qb1][$xlab_qb2]))
                     {
                         $this->array_datay[$xlab_qb2][] =  $this->array_total_id_competencia[$lab_qb1][$xlab_qb2][$this->campo];
                     }
                     else
                     {
                         $this->array_datay[$xlab_qb2][] = 0;
      ...
      if ($this->nivel == 2)
      {
         $this->array_label = $this->array_label_geral[$this->nivel][$this->id_votado][$this->id_etapa];
         $this->sc_graf_sint = true;
         $this->array_datay[0] = $this->array_datay_geral[$this->nivel][$this->campo][$this->id_votado][$this->id_etapa];
      }
   ...
   function grafico_generico($tipo, $width, $height, $margem, $aspecto, $preenche, $exibe_val, $exibe_marc, $datay, $label, $tit_datay, $tit_label, $tit_graf, $tip_pizza = "ABS")
   {
      ...
      if (empty($datay) && $_SESSION['sc_session'][$this->Ini->sc_page]['app_Rel']['opcao'] == "pdf")
      {
         return;
      }
