...
class app_PartPistaCompHab_grafico
{
   var $Db;
   var $Ini;
   var $Erro;
   ...
   //---- Construtor
   function app_PartPistaCompHab_grafico()
   {
   ...
   function monta_grafico()
   {
      ...
      {
          $graf_tipo    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_tipo'];
          $graf_alt     = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_alt'];
          $graf_larg    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_larg'];
          $graf_marg    = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_margem'];
          $graf_asp     = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_aspec'];
          $graf_preen   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_preencher'];
          $graf_ex_val  = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_exibe_val'];
          $graf_ex_marc = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_exibe_marcas'];
          $this->graf_cor_fundo   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_cor_fundo'];
          $this->graf_cor_margens = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_cor_margens'];
          $this->graf_cor_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_cor_label'];
          $this->graf_cor_valores = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_cor_valores'];
          $this->graf_tipo_marcas = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_tipo_marcas'];
          $this->graf_angulo      = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_angulo'];
   ...
   {
      global 
             $nivel_quebra, $campo, $participante, $pista, $competencia;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['graf_opc_atual'] == 1)
      {
         $this->sc_graf_sint = true;
      }
      //---- Total
      require_once($this->Ini->path_aplicacao . "app_PartPistaCompHab_total.class.php"); 
      $this->total = new app_PartPistaCompHab_total($this->Ini->sc_page);
      $this->prep_modulos("total");
      //---- Parâmetros
      if (get_magic_quotes_gpc())
      {
         $participante = stripslashes($participante);
         $pista = stripslashes($pista);
         $competencia = stripslashes($competencia);
      }
      $participante = urldecode($participante);
      $pista = urldecode($pista);
      $competencia = urldecode($competencia);
      $this->campo = (isset($campo))        ? $campo        : 0;
      $this->nivel = (isset($nivel_quebra)) ? $nivel_quebra : 0;
      $this->participante = (isset($participante)) ? $participante : "";
      $this->pista = (isset($pista)) ? $pista : "";
      $this->competencia = (isset($competencia)) ? $competencia : "";
      //---- Arrays de totais
      ...
      if (0 < $this->nivel)
      {
         $this->titulo .= " - Participante = $conteudo";
      }
      $conteudo = $pista;
      $pista = $pista;
      if (1 < $this->nivel)
      {
         $this->titulo .= " - Pista = $conteudo";
      }
      //---- Label
      $this->label    = array();
      $this->label[0] = "Participante";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['participante']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['participante']))
      {
         $this->label[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['participante'];
      }
      $this->label[1] = "Pista";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['pista']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['pista']))
      {
         $this->label[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['pista'];
      }
      $this->label[2] = "Competência";
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['competencia']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['competencia']))
      {
         $this->label[2] = $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['contr_label_graf']['competencia'];
      }
   }
   ...
   function monta_arrays()
   {
      ...
      $this->total->resumo("gra", $this->array_total_participante, $this->array_total_pista, $this->array_total_competencia);
      ksort($this->array_total_participante);
      ksort($this->array_total_pista);
      ksort($this->array_total_competencia);
      $this->total->grafico($this->array_label_geral, $this->array_datay_geral, $this->array_total_participante, $this->array_total_pista, $this->array_total_competencia);
      if ($this->nivel == 0)
      {
         ...
         else
         {
             $nn = $this->nivel + 1;
             $lab_qb2 = array();
             foreach ($this->array_label_geral[$nn] as $qb => $lab)
             {
                 foreach ($lab as $cada_lab)
                 {
                     if (!isset($lab_qb2[$cada_lab]))
                     {
                         $lab_qb2[$cada_lab] = $cada_lab;
                     }
                 }
             }
             ksort($lab_qb2);
             foreach ($this->array_label as $lab_qb1)
             {
                 foreach ($lab_qb2 as $xlab_qb2)
                 {
                     if (isset($this->array_total_pista[$lab_qb1][$xlab_qb2]))
                     {
                         $this->array_datay[$xlab_qb2][] =  $this->array_total_pista[$lab_qb1][$xlab_qb2][$this->campo];
                     }
                     else
                     {
                         $this->array_datay[$xlab_qb2][] = 0;
                     }
                 }
             }
         }
      ...
      if ($this->nivel == 1)
      {
         $this->array_label = $this->array_label_geral[$this->nivel][$this->participante];
         if ($this->sc_graf_sint)
         {
             $this->array_datay[0] = $this->array_datay_geral[$this->nivel][$this->campo][$this->participante];
         }
         else
         {
             $nn = $this->nivel + 1;
             ...
             foreach ($this->array_label as $lab_qb1)
             {
                 foreach ($lab_qb2 as $xlab_qb2)
                 {
                     if (isset($this->array_total_competencia[$lab_qb1][$xlab_qb2]))
                     {
                         $this->array_datay[$xlab_qb2][] =  $this->array_total_competencia[$lab_qb1][$xlab_qb2][$this->campo];
                     }
                     else
                     {
                         $this->array_datay[$xlab_qb2][] = 0;
      ...
      if ($this->nivel == 2)
      {
         $this->array_label = $this->array_label_geral[$this->nivel][$this->participante][$this->pista];
         $this->sc_graf_sint = true;
         $this->array_datay[0] = $this->array_datay_geral[$this->nivel][$this->campo][$this->participante][$this->pista];
      }
   ...
   function grafico_generico($tipo, $width, $height, $margem, $aspecto, $preenche, $exibe_val, $exibe_marc, $datay, $label, $tit_datay, $tit_label, $tit_graf, $tip_pizza = "ABS")
   {
      ...
      if (empty($datay) && $_SESSION['sc_session'][$this->Ini->sc_page]['app_PartPistaCompHab']['opcao'] == "pdf")
      {
         return;
      }
